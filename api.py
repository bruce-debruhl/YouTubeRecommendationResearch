...
DEVELOPER_KEY = 'AIzaSyAkdM2yfWSIeVv-aVd5rrDojhWCw15lMyw'
...
# request for videoData for a given 'videoID'
def vid_info_request(videoID):
    print("Getting vid info for ID: ", videoID)
...
# unsure if this function is used, vidToJSON_forDB() is almost an exact copy
def vidToJSON(vid):
    return {
         "id": vid.id,
         "title": vid.title,
...
# convert Video Object to JSON format for Database Entry
...
# insert Video Object Data into Database
...
# only append first NUM_REC_VIDS of videoIDs to recommendation list
...
# request related/next step video recommendations up to 30 for a given videoID
...
# return list of topLevelComments for given 'videoID'
def fill_comments(videoID):
    cmts = []
    response = comments_request(videoID)

    # see ./response_formats/comments.json for structure
...
# request comments from a given 'videoID' in order of relevance
...
def level_order_traversal(videoID):
    ...
    while(count <= NUM_VIDS):
        ...
        print("Removed parent from q: ", parentID)

        myquery = {"id": parentID}
        mydoc = COL.find(myquery)

        # if video doesn't exist in the DB
        # then enter parentID data into DB
        if(mydoc.count() == 0):
            parentVid = get_video_info_and_add_to_DB(parentID)
            print('Added parent to DB')
            childrenList = parentVid.recs

            # add children of parent VideoID to Queue
            for child in childrenList:
                q.put(child[1])
                print('Added child to q: ', child[1])

        count += 1

    queueToFile(q)

# write videoID to 'queue.txt' file
def queueToFile(q):
    while(True):
        try:
...
# add video in file - 'queue.txt' to Queue
def fileToQueue(q):
